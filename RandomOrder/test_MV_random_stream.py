...
def main():
  ...
  with open('MV_setup.txt') as parameters_file:
  ...
  program_dir = "./gbbs/benchmarks/McGregorVorotnikova20/"
  ...
  for file_idx, filename in enumerate(files):
    for program_idx, program in enumerate(programs):
      for e in eps:
        num_rounds = 1
        out_path_components = [programs[program_idx], filename, e, ".out"]
        out_filename = os.path.join(write_dir, "_".join(out_path_components))
        program_path = os.path.join(program_dir, program)
        ss = (program_path + " -i " + read_dir + filename + " -eps " + e + " "
                "-rounds 1 " + empty)
        print(ss)
