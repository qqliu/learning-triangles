...
struct TrianglesPerVertex {
    ...
    size_t _n;
    double _error;
    symmetric_graph<symmetric_vertex, weight_type> _graph;
    ...
    TrianglesPerVertex(size_t n, double error, symmetric_graph<symmetric_vertex, weight_type> graph):
        _n(n), _error(error), _graph(graph) {}
    ...
    std::pair<size_t, size_t> CountTrianglesPerVertex(
        ...
        /*auto contains_a_triangle = gbbs::make_sparse_table<uintE, bool>(1 << 20,
                ...
                std::hash<uintE>{});*/
        auto edges = sequence<std::pair<uintE, uintE>>(end - start);
        ...
        parallel_for(start, end, [&](size_t jk) {
            ...
            if (min_id != max_id) {
                edges[jk] = std::make_pair(min_id, max_id);
        ...
        auto counts = sequence<uintE>((end-start), 0);
        auto bucket_counts = sequence<uintE>((end-start), 0);
        auto has_triangle = sequence<uintE>((end-start), 0);

        parallel_for(0, edges.size(), [&](size_t i) {
            auto edge = edges[i];
            auto our_neighbors = _graph.get_vertex(edge.first).out_neighbors();

            auto their_neighbors = _graph.get_vertex(edge.second).out_neighbors();
            auto intersect_count = our_neighbors.intersect_f_par(&their_neighbors, f);

            if (intersect_count > 0)
                has_triangle[i] = 1;

            auto bucket = ceil(log(intersect_count)/log(_error));
            counts[i] = (new_edges.size()/((1.0) * (end - start)))
                    * ceil(pow(_error, bucket));

            bucket_counts[i] = bucket;
        });
        ...
        /*parallel_for(0, starts.size()-1, [&](size_t jn) {
            ...
            parallel_for(starts[jn], starts[jn+1], [&] (size_t jk) {
                ...
                cur_count *= new_edges.size() / pow(end, 1.5); //((num_edges_considered) / _n);
                counts[jk] = (new_edges.size()/((1.0) * (end - start)))
                    * ceil(pow(_error, bucket));
                bucket_counts[jk] = bucket;
                if (cur_count > 0)
                    has_triangle[jn] = 1;
            });
    ...
    /*
        ...
        */
        parlay::sort_inplace(parlay::make_slice(bucket_counts));
        auto bool_seq_bucket = parlay::delayed_seq<bool>(bucket_counts.size() + 1,
                [&] (size_t i) {
                    return (i == 0) || (i == bucket_counts.size())
        ...
        auto starts_bucket = parlay::pack_index(bool_seq_bucket);
        //auto reverse_starts = parlay::pack_index(bool_seq_reverse);

        for (size_t i = 0; i < starts_bucket.size() - 1; i++) {
            std::cout << " (" << bucket_counts[starts_bucket[i]] << ", " <<
                starts_bucket[i+1] - starts_bucket[i] << ") ";
        }
...
inline void ApproximateTriangles (BatchDynamicEdges<W>& batch_edge_list,
        ...
        size_t num_vertices,
        double approx_error,
        bool randomTrial = false,
        size_t vertex_sample_size = 1,
        bool randomStream = false){
    auto batch = batch_edge_list.edges;
    auto rng = std::default_random_engine {};

    size_t num_edges = batch.size();
    // uintE cutoff = multiplier * ceil(log(num_edges));
    uintE edge_percent = floor(percent * num_edges);

    std::vector<double> errors;

    for (size_t trial = 0; trial < trials; trial++) {
        if (randomTrial) {
            std::cout << "### Random Stream ###" << std::endl;
        ...
        auto whole_graph = dynamic_edge_list_to_symmetric_graph(
                batch_edge_list.edges, batch.size());
        ...
        for (size_t i = 0; i < batch.size(); i++) {
            if (randomTrial) {
                ...
                if (i >= edge_percent || i == batch.size() - 1) {
                //if (num_to_sample == 0) {
                    //std::cout << "cur start, i: " << start << ", " << i << std::endl;
                    auto end_size = std::min(i, batch.size());
                    auto graph = dynamic_edge_list_to_symmetric_graph(batch, end_size);
                    TrianglesPerVertex count_struct =
                        TrianglesPerVertex<W>((size_t)num_vertices,
                            (double) approx_error, graph);
                    auto outs = count_struct.CountTrianglesPerVertex(batch, start,
                        std::min((i + 1), num_edges));
                    //std::cout << "cur start, i: " << start << ", " << i << std::endl;
                    //batch_edge_list);
                    //, 0, i, P);
                    ...
                    auto results = Triangle(graph, f, "kcore", P);
                    std::cout << "CORRECT COUNT: " << results.first
                        << ", calculated count: " << outs.first/3 << std::endl;

                    approx_count = triangle_estimate / 3;
                    break;
        ...
        error = std::abs(1 - ((1.0 * approx_count) / triangle_real_count));
            /*(1.0 * std::max(triangle_real_count,
